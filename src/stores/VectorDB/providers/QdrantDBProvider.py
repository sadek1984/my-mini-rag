...
class QdrantDBProvider(VectorDBInterface):  
    ...
    async def create_collection(self, collection_name: str, 
        ...
        # Check if collection exists with wrong size
        ...
        if collection_exists:
            ...
            if current_size != embedding_size:
                self.logger.warning(f"Vector size mismatch! Existing: {current_size}, Required: {embedding_size}")
                await self.delete_collection(collection_name)
                collection_exists = False

        if not collection_exists:
            self.logger.info(f"Creating collection {collection_name} with size {embedding_size}")
            self.client.create_collection(
                collection_name=collection_name,
                vectors_config=models.VectorParams(
                    size=embedding_size,  # Use actual size

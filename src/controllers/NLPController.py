...
# 🔥 ADD THIS DEBUGGING:
        ...
        print(f"About to embed {len(texts)} texts")
        print(f"This will make {len(texts)} Cohere API calls")
        print(f"Embedding model: {self.embedding_client.embedding_model_id}")
        print(f"API key ends with: ...{self.embedding_client.api_key[-8:] if self.embedding_client.api_key else 'None'}")

         # FIXED: Pass individual text, not the entire texts list
        raw_vectors = [self.embedding_client.embed_text(text=text, 
